...
export class TrekAddComponent implements OnInit, OnDestroy {
  ...
  async onExcelFileSelect(event: any) {
    ...
    try {
      ...
      this.notificationService.show('Failed to read Excel file. Please check the format and try again.', 3500);
    } finally {
  ...
  onCoverImageSelect(event: any) {
    ...
    if (!this.isAcceptedImage(file)) {
      this.setStatus('Please select a valid image file', 'warning');
      this.notificationService.show('Please select a valid image file');
      return;
    }
  ...
  .subscribe({
    ...
    error: (err) => {
      ...
      this.setStatus('Server error. Please try again.', 'danger');
